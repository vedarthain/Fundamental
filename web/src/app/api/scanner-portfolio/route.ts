/**
 * /api/scanner-portfolio — read/write the Scanner "P" tags (Graph tab).
 *
 *   GET    — returns { signedIn, symbols } for the signed-in user's tags.
 *            Signed-out returns { signedIn: false, symbols: [] } so the
 *            client knows to fall back to localStorage.
 *   POST   — body { symbol } or { symbols: [...] } — adds tags (batch form
 *            is used by the login/signup merge).
 *   DELETE — ?symbol=X — removes a tag.
 *
 * A sibling of /api/scanner-favourites — same membership-only shape. This is
 * a scanner-view marker, NOT real holdings (those live in app.portfolio_holding
 * and are managed on the Portfolio tab). Signed-out POST/DELETE return 401; the
 * client uses localStorage anonymously and never calls those routes.
 *
 * Cost (Rule #1):
...
export async function GET() {
  ...
  const rows = await sql<{ symbol: string }[]>`
    SELECT symbol FROM app.user_scanner_portfolio
...
export async function POST(req: NextRequest) {
  ...
  await sql`
    INSERT INTO app.user_scanner_portfolio (user_id, symbol)
...
export async function DELETE(req: NextRequest) {
  ...
  await sql`
    DELETE FROM app.user_scanner_portfolio
     WHERE user_id = ${session.userId}
       AND symbol  = ${sym}
  `;
