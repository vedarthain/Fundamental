...
// ── Split/bonus reconciliation helpers (shared) ────────────────────────────
// Trades/holdings are stored RAW (as executed at the broker); golden's
// price_history is (usually) split/bonus-ADJUSTED. Left unreconciled, a
// post-trade corporate action makes the B/S marker label + cost basis read ~Nx
// the chart. We scale a price by the cumulative split_factor for ex-dates AFTER
// the reference date, arbitrated by golden's own close on that date (keep
// whichever of {raw, raw×factor} sits closer). Adjusted wins where golden is
// back-adjusted; raw wins where it isn't — a no-op for inconsistent tables.
// Factored out so both loadPortfolioTrades and loadSnapshotDerivedBuys share
// one reconciliation path (and its action/close caches within a request).
function makeReconciler() {
  ...
  // Only fetched for entries that actually have a corporate action (factor ≠ 1).
  ...
  return { cumFactor, loadActions, loadCloses, reconcile };
}

// Synthesise "B" markers for snapshot-only holdings (broker snapshot with no
// trade log): the historical bar (on or before import) whose split-adjusted
// price is nearest the avg cost. Flagged `derived` — approximate by construction.
...
export async function loadSnapshotDerivedBuys(
  ...
  if (snapOnly.length === 0) return {};
  const { cumFactor, loadActions, loadCloses, reconcile } = makeReconciler();
  await loadActions(snapOnly.map((s) => s.symbol));
  // Broker snapshots already reflect corporate actions up to the import date;
  // golden is adjusted to today. Reconcile the residual — actions after import —
  // arbitrated by golden's close on the import date.
  await loadCloses(
    snapOnly
  ...
  for (const s of snapOnly) {
    ...
    out[s.symbol.toUpperCase()] = [
      { d, side: "B", price: snapAdj(s), qty: Math.round(s.qty), derived: true },
    ];
  }
  return out;
}
...
export async function loadPortfolioTrades(
  ...
  // candles near ₹1,960. The shared reconciler (makeReconciler) scales a trade's
  // price by the cumulative split_factor for ex-dates AFTER it, arbitrated by
  // golden's close on the trade date. See its doc comment for the full rationale.
  const { cumFactor, loadActions, loadCloses, reconcile } = makeReconciler();
  ...
  // Synthetic buys for snapshot-only holdings (broker snapshot, no trade log) —
  // guarantees every held name shows an entry point. See loadSnapshotDerivedBuys.
  const derived = await loadSnapshotDerivedBuys(userId);
  for (const [sym, marks] of Object.entries(derived)) {
    (tradesBySymbol[sym] ??= []).push(...marks);
  }
